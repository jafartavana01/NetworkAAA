...
_SECTION_BY_PATH: dict[str, tuple[str, list[str]]] = {
    ...
    "/tacacs/access-events": ("tacacs", ["access events", "authentication", "login failures", "denied", "permit", "who logged in"]),
    ...
    "/platform/license": ("system", ["license", "licence", "edition", "subscription", "community", "upgrade", "device limit"]),
}
...
def _module_is_licensable(module) -> bool:
    """True when some feature gates this module. A module no feature
    mentions (Platform, System) is never hidden by licensing -- only
    modules the edition table actually governs."""
    from ..services.editions import FEATURES

    return any(f.module_key == module.key for f in FEATURES)


def build_sidebar_sections(
    modules: list[Module], *, is_superadmin: bool, licensed_module_keys: set | None = None,
) -> tuple[SidebarItem | None, list[SidebarSection]]:
    """
    ...
    # Hide modules the licence does not include. A link that leads
    # straight to a 402 is worse than no link: it reads as a broken
    # page rather than as a product boundary. The API is still gated
    # independently -- this is presentation, never the control.
    if licensed_module_keys is not None:
        modules = [
            module for module in modules
            if module.mandatory or module.key in licensed_module_keys
            or not _module_is_licensable(module)
        ]
