...
#: The same timestamp WITHOUT the trailing-whitespace requirement.
#:
#: `_TIMESTAMP_PATTERN` ends in `\s+` so it can find where the prefix
#: stops inside a full line -- but `group(1)` excludes that whitespace.
#: Re-matching the captured prefix against the original pattern
#: therefore always failed, and every record parsed with
#: `_try_parse_timestamp(raw_prefix)` silently got `parsed_at = None`.
#: The accounting parser happened to pass the WHOLE line and so was
#: unaffected, which is why this survived: the bug only appeared in the
#: parsers written later, and only in a column ("Time") that shows an
#: em dash rather than an error when empty.
_TIMESTAMP_ONLY_PATTERN = re.compile(r"^(\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}:\d{2}\s+[+-]\d{4})")


def _try_parse_timestamp(prefix: str) -> datetime | None:
    """Accepts either a bare timestamp or a full line beginning with one."""
    match = _TIMESTAMP_ONLY_PATTERN.match(prefix or "")
    if not match:
        return None
    try:
...
def _parse_line(line: str, separator: str = ACCOUNTING_FIELD_SEPARATOR) -> AccountingRecord:
    """Parses one log line. `separator` is a parameter because the
    accounting log and the authorization log use the SAME eight
    fields and the same timestamp prefix, differing only in their
    delimiter. They were separate near-identical copies; a fix to
    one would silently have missed the other, which is the classic
    way parsing bugs come back.
    """
    line = line.rstrip("\n")
    ...
    raw_prefix = separator.join(parts[:-_EXPECTED_FIELD_COUNT]).strip()
...
#: The authorization log carries a VARIABLE number of fields, confirmed
#: against a real deployment:
#:
#:   7 fields  session (shell login) authorization -- no command
#:   8 fields  per-command authorization
#:
#:   ts  nas  user  port  rem_addr  priv  result  service  [cmd]
#:
#: Note this is NOT the accounting log's field list: positions 4 and 5
#: hold the remote-address type and privilege level, not `nac` and
#: `accttype`. Only nas/user/port/result/service/cmd are meaningful
#: here, and those are what this parser fills.
_AUTH_FIELD_COUNT_WITH_COMMAND = 8
_AUTH_FIELD_COUNT_SESSION = 7


#: Command patterns whose ARGUMENT is a credential. The authorization
#: log records the command exactly as typed, so a line like
#: `username 11 password 22` lands in it in CLEARTEXT -- confirmed
#: against a real deployment, where the accounting log masked the same
#: command as `password *****` and the authorization log did not.
#:
#: Anything surfaced from this log therefore has to be masked here,
#: before it reaches an API response, a CSV export or a browser. The
#: raw file still contains the secret; that is a file-permissions
#: matter, but the platform should not be the thing that copies it onto
#: someone's screen.
_SECRET_ARG_PATTERNS = [
    re.compile(r"(?i)\b(password)\s+(\S+)"),
    re.compile(r"(?i)\b(secret)\s+(\S+)"),
    re.compile(r"(?i)\b(key)\s+(\S+)"),
    re.compile(r"(?i)\b(community)\s+(\S+)"),
    re.compile(r"(?i)\b(md5)\s+(\S+)"),
]

#: Tokens that are a keyword rather than a secret, so masking them
#: would hide the command's meaning without protecting anything.
_NOT_A_SECRET = {"encryption", "encrypted", "authentication", "authorization", "level", "0", "7"}


def mask_credentials(command: str) -> str:
    """Replaces a credential argument with a fixed mask."""
    if not command:
        return command
    masked = command
    for pattern in _SECRET_ARG_PATTERNS:
        def _replace(match):
            ...
            if value.lower() in _NOT_A_SECRET:
...
def _auth_parse_line(line: str) -> AccountingRecord:
    """
    Parses one authorization-log line.

    Written separately from `_parse_line` after testing against a REAL
    log: the two formats differ by more than a delimiter. A session
    authorization has seven fields and no command, so reusing the
    accounting parser -- which requires exactly eight -- silently
    failed on every login record and returned `parsed=False`. That is
    precisely the record an access-events view needs, so the failure
    would have shown an empty page rather than an error.
    """
    line = line.rstrip("\n").rstrip("\r")
    ...
    # Tolerate a run of spaces where a tab was expected: some terminals
    # and copy-paste paths convert them, and refusing to parse a record
    # because of whitespace would lose real access decisions.
    if len(parts) < _AUTH_FIELD_COUNT_SESSION:
        parts = [p for p in re.split(r"\t+| {2,}", payload) if p != ""]

    if len(parts) < _AUTH_FIELD_COUNT_SESSION:
        return AccountingRecord(raw_line=line, parsed=False)

    nas, user, port, rem_addr, priv, result, service = parts[:_AUTH_FIELD_COUNT_SESSION]
    # Anything beyond the seven fixed fields is the command, which may
    # itself contain the separator -- so it is rejoined rather than
    # truncated at the first one.
    cmd = AUTH_FIELD_SEPARATOR.join(parts[_AUTH_FIELD_COUNT_SESSION:]).strip()

    return AccountingRecord(
        raw_line=line,
        parsed=True,
        raw_prefix=raw_prefix,
        parsed_at=_try_parse_timestamp(raw_prefix or line),
        nas=nas.strip(),
        user=user.strip(),
        port=port.strip(),
        nac=rem_addr.strip(),
...
#-----------------------custom Reader -------------------------
def read_auth_records(*, limit: int = 500, include_logins: bool = False) -> list[AccountingRecord]:
    """
    Authorization-log records, newest first.

    A record with an EMPTY `cmd` is the initial shell (login)
    authorization -- "may this user get a session on this device at
    all" -- rather than a per-command decision. The command views drop
    those, because a blank command in a command list is noise.

    `include_logins=True` keeps them, which is what a login/authentication
    view needs: the permit or deny on that record IS the access
    ...
    records = [
        ...
        if include_logins or record.cmd != ""
    ]
    records.reverse()  # newest first
    return records


def is_login_record(record: AccountingRecord) -> bool:
    """A session (shell login) authorization rather than a command one."""
    return record.parsed and not record.cmd


def login_outcome(record: AccountingRecord) -> str:
    """
    "success" | "failure" | "unknown" for a login authorization.

    tac_plus-ng writes the decision in the `result` field. Anything
    that is not recognisably a permit is reported as a FAILURE rather
    than assumed successful -- an unreadable access decision must never
    read as "access granted". A record with no result at all is
    "unknown", which is distinct from both.
    """
    value = (record.result or "").strip().lower()
    if not value:
        return "unknown"
    return "success" if value.startswith("permit") else "failure"
...
# ---------------------------------------------------------------------
# Access log -- the real AUTHENTICATION record
# ---------------------------------------------------------------------
#
# Format confirmed against a real deployment:
#
#   2026-09-13 05:12:20 +0000 192.168.44.10\tjafar\ttty0\tasync\tshell login failed
#
#   ts  nas  user  port  rem_addr  <message>
#
# Five tab-delimited fields followed by a free-text message. This is the
# AUTHENTICATION outcome -- whether the credential check itself passed --
# which neither the accounting log (no result at all) nor the
# authorization log (which records what an already-authenticated session
# was allowed to do) can tell you.
ACCESS_LOG_PATH = LOG_DIR / "tac_plus-ng-access.log"
ACCESS_FIELD_SEPARATOR = "\t"
#: nas, user, port, rem_addr -- four identity fields, then the message.
#: Counted from the real log rather than assumed: an earlier value of 5
#: required six parts and rejected every genuine line.
_ACCESS_FIXED_FIELDS = 4
...
class AccessRecord:
    ...
    def outcome(self) -> str:
        ...
        Derived from the message tail, and deliberately conservative: a
        message that does not clearly say it succeeded is NOT reported
        as a success. An authentication record the platform cannot read
        must never render as "logged in successfully".
        """
        text = (self.message or "").strip().lower()
        if not text:
            return "unknown"
        if "succeed" in text or text.endswith("success"):
            return "success"
        if "fail" in text or "denied" in text or "reject" in text or "error" in text:
            return "failure"
        return "unknown"


def _access_parse_line(line: str) -> AccessRecord:
    line = line.rstrip("\n").rstrip("\r")
    ...
    parts = payload.split(ACCESS_FIELD_SEPARATOR)
    if len(parts) < _ACCESS_FIXED_FIELDS + 1:
        # Same whitespace tolerance as the authorization parser: a run
        # of spaces where a tab was expected should not lose a real
        # authentication record.
        parts = [p for p in re.split(r"\t+| {2,}", payload) if p != ""]

    if len(parts) < _ACCESS_FIXED_FIELDS + 1:
        return AccessRecord(raw_line=line, parsed=False)

    nas, user, port, rem_addr = parts[:4]
    # Everything after the four identity fields is the message. It is
    # rejoined rather than truncated, because the message is free text
    # and may itself contain the separator.
    message = ACCESS_FIELD_SEPARATOR.join(parts[4:]).strip()

    return AccessRecord(
        raw_line=line, parsed=True, raw_prefix=raw_prefix,
        parsed_at=_try_parse_timestamp(raw_prefix or line),
        nas=nas.strip(), user=user.strip(), port=port.strip(),
        rem_addr=rem_addr.strip(), message=message,
    )


def read_access_records(*, limit: int = 500) -> list[AccessRecord]:
    """Authentication records, newest first."""
