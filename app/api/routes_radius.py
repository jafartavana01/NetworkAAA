...
class RadiusListenerOut(BaseModel):
    port: int
    listening: bool
    addresses: list[str]
    all_interfaces: bool
...
@router.get("/listeners", response_model=list[RadiusListenerOut])
def radius_listeners(
    db: Session = Depends(get_db),
    _admin: AdminUser = Depends(get_current_superadmin),
):
    """
    Whether anything is actually listening on the RADIUS ports, and on
    which address.

    This answers a question no amount of RADIUS configuration can: a
    daemon bound only to LOOPBACK answers a local probe perfectly while
    being unreachable from every other host, so a NAS gets nothing and
    its packets never even reach a capture on this server.
    """
    from ..services import radius_probe

    settings = _get_or_create(db)
    out = []
    for port in (settings.auth_port, settings.acct_port):
        status_result = radius_probe.listener_status(port)
        out.append(RadiusListenerOut(
            port=port, listening=status_result.listening, addresses=status_result.addresses,
            all_interfaces=status_result.all_interfaces, loopback_only=status_result.loopback_only,
            detail=status_result.detail, guidance=status_result.guidance,
        ))
    return out
