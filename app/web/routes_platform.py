...
@router.get("/platform/license", response_class=HTMLResponse)
def license_page(
    request: Request,
    session_token: str | None = Cookie(default=None, alias=security.SESSION_COOKIE_NAME),
):
    # Superadmin-only, matching the API: entitlements govern the whole
    # installation.
    return _render(request, session_token, "license.html", require_superadmin=True)
