...
def _existing_checkout_commit() -> str | None:
    """The commit an existing checkout sits at, or None if there isn't
    a usable one. A directory without `.git`, or one git cannot read,
    counts as absent rather than as a broken checkout to repair."""
    if not (UPSTREAM_SRC_DIR / ".git").exists():
        return None
    try:
        result = utils.run(
            ["git", "-C", str(UPSTREAM_SRC_DIR), "rev-parse", "HEAD"], check=False
        )
    except Exception:
        return None
...
def clone_or_update_upstream(*, assume_yes: bool = False) -> str:
    """
    Clone the upstream repo, or reuse an existing checkout.

    When a checkout is already present the operator is ASKED whether to
    reuse it, defaulting to yes. Re-downloading by default is the wrong
    behaviour on this kind of install: the clone is the slowest step,
    it needs internet on a box that may not have it, and a reinstall or
    a second run is the common case rather than the exception.

    Answering no fetches and hard-resets to origin/master, which is the
    previous behaviour and remains available for deliberately taking
    upstream changes.
    """
    UPSTREAM_SRC_DIR.parent.mkdir(parents=True, exist_ok=True)

    existing = _existing_checkout_commit()
    if existing:
        utils.info(f"Upstream source already present at {UPSTREAM_SRC_DIR}")
        utils.info(f"  currently at commit {existing[:12]}")
        reuse = True if assume_yes else utils.confirm(
            "Use the existing source instead of downloading it again?", default_yes=True
        )
        if reuse:
            utils.ok("Reusing the existing upstream checkout -- nothing downloaded.")
            return existing

        utils.info("Fetching the latest upstream source ...")
