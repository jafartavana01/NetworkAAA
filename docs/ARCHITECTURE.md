...
# Accounting vs authorization logs

tac_plus-ng writes command activity to two different files, and they
do not contain the same thing:

* **`tac_plus-ng-accounting.log`** -- `::`-delimited, format defined by
  this project's own `accounting format` directive. Contains commands
  that were ALLOWED to run. It does not record a denial, and does not
  state that a recorded command was permitted; that is simply implied
  by its presence.
* **`tac_plus-ng-authorization.log`** -- TAB-delimited, same eight
  fields. Contains the authorization DECISION, permit and deny alike.

For any view that asks "what did this user try, and what happened",
the authorization log is the better source: the accounting log cannot
show a denial at all, so a dashboard built on it silently reports a
world in which nothing is ever refused.

`read_auth_records()` reads the authorization log and keeps only
records with a non-empty `cmd`, so per-command decisions are shown
without the shell-login records that carry no command.

`_parse_line(line, separator)` serves both files. They were briefly
two near-identical parsers differing only by delimiter; a fix to one
would silently have missed the other, which is how parsing bugs come
back. Behavioural equivalence with the separate versions was verified
across accounting lines, authorization lines, lines with no timestamp
prefix, partial-field lines and unparseable input.

**Known limitation.** The authorization log's tab-delimited layout is
observed from a real deployment, not set by a directive this project
emits -- unlike the accounting format, which is declared in the
generated configuration. If a future tac_plus-ng release changes that
default layout, parsing would need revisiting. Declaring the format
