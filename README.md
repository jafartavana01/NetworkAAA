<img width="1601" height="795" alt="image" src="https://github.com/user-attachments/assets/f499df5a-8620-4f1f-ac92-fd7bfb75d639" />
...
| **Configuration Management (NCM)** *(new)* | Configuration backup, versioning, diff and scheduled backups as a first-class subsystem. Immutable SHA-256-hashed snapshots (a new version is created only when the configuration actually changed, while the backup execution itself is always recorded); per-device version history with a line-numbered, searchable configuration viewer, copy and download; unified text diff between any two versions of a device, with an explicit "no previous configuration available" state rather than a fabricated comparison; recurring daily backup schedules targeting devices and/or device groups (deduplicated), persisted server-side and executed without the browser open; backup jobs with per-device results distinguishing connection failure from retrieval failure, and "new version archived" from "unchanged, nothing archived". Reuses the platform's existing SSH execution, device groups, encrypted service account and concurrency limits — no second SSH implementation or credential store. Deliberately separate from Command Sets and tac_plus-ng compilation: NCM answers "how is this device's configuration backed up and versioned", not "which commands may this user run". See `docs/ARCHITECTURE.md` for the full design, current limitations and future phases |
