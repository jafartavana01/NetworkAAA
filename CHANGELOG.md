...
an optional reference, then downloads the `.request` file. That removes
the three steps most likely to leave "how do I buy this?" unanswered:
find a shell, find the install directory, run a script.

**One limitation is reported, not hidden.** The service account cannot
read the DMI system UUID -- that needs root -- so a request generated
in the browser carries one fewer identifying component than one from
`sudo python3 collect_fingerprint.py`. Two are enough to bind a
licence, so the GUI route is usually fine. When it is NOT, the dialog
says so and points at the script, rather than letting the customer
discover it when their licence fails to verify. The count is returned
as a response header so the page can warn without re-parsing the file
it just downloaded.

**Interactive issuing.** Running `issue_license.py` with no arguments
now asks for customer name, licence level (as a numbered menu showing
each tier's device count), device override, validity in DAYS rather
than a date to calculate, request file and signing seed.

Every answer is echoed for confirmation before anything is signed. A
licence is in the customer's hands by the time a wrong expiry or device
count is noticed, so it is worth one keystroke to catch.

`--batch` never prompts and fails instead, so scripted issuing cannot
hang waiting on input.

Verified: the interactive path produces a genuinely valid licence
(checked by verifying the output, not by trusting the script said
"Wrote"); cancelling writes no file; supplying a real seed removes the
development-key warning; and batch mode fails fast.

---

### Changed — production verification key installed; issuer is now standalone

**`PRODUCTION_PUBLIC_KEY` is set.** The supplied key was validated as a
real 32-byte Ed25519 public key before being installed, and confirmed
to reject signatures from any other key. With it in place, licences
signed with the published development seed **stop verifying** --
demonstrated, not assumed.

The test suite pins itself to the development key for the duration of
its run, because it must keep exercising the licensing logic on a build
that ships a real key no test can sign for. Two new assertions cover
what that pinning could otherwise hide: that
`PRODUCTION_PUBLIC_KEY` is set at all, and that it is not the
development key.

**The issuer no longer needs the project directory.** It imported the
edition table, the product id and the request decoder from the
application, which forced it to live in `tools/`. That was convenient
and wrong -- the one file that must never travel with the software was
sitting inside the software. It now imports nothing from the project
and runs from anywhere: an offline laptop, a USB key, a vault.

The duplication that buys is small and safe by construction: the issuer
writes device limits and features INTO the signed licence, and the
application reads them from the signature rather than from its own
edition table. If the two ever drift, a customer's entitlements still
match what they were sold. `--devices` overrides the table entirely for
any agreement the tiers do not describe.

Verified end to end from `/tmp` with no project present: a keypair
generated, a licence signed with the private seed, and that licence
confirmed to verify against the matching public key, to carry no
development-key warning, and to be REJECTED against a different public
key.

---

### Changed — uninstall output now states plainly what is kept and what is removed

My earlier wording ("the checkout's .git and contents survive") read as
though only `.git` was kept and raised a fair question about whether
the tac_plus-ng service was still being removed.

Nothing was wrong in the behaviour -- confirmed by inspection and test:

* **Both systemd services are still fully removed** -- stopped,
  disabled, unit files deleted, `daemon-reload` run. The change only
  ever touched directory removal.
* **The installed binary is still removed.** It lives in
  `/usr/local/sbin`, outside the preserved directory.
* **The ENTIRE source tree is kept**, not just `.git` -- verified
  against a realistic checkout containing `.git`, `configure`,
  `PREREQUISITES.txt`, C sources and the `sample/` configs. All nine
  entries survive; `app/`, `venv/` and `generated/` are removed.

What was wrong was the message, so that is what changed. The removal
plan now says explicitly:

    NOT REMOVED -- the downloaded tac_plus-ng SOURCE CODE only: /opt/...
       (the service, its unit file and the installed binary ARE removed;
        only the source tree is kept so the next install skips the
        download. Use --purge-upstream to delete it too.)

and the closing summary lists what WAS removed rather than only what
was not, so an operator does not have to infer it.

---

### Changed — installer reuses an existing upstream checkout; uninstall keeps it

**On install**, when the upstream source checkout is already present
the operator is asked whether to reuse it, **defaulting to yes**:

    Upstream source already present at /opt/aaa-platform/upstream/...
      currently at commit 3a7b593f202a
      Use the existing source instead of downloading it again? [Y/n]:

Re-downloading by default was the wrong behaviour here. The clone is
the slowest step, it needs internet on a box that may not have any, and
a reinstall is the common case rather than the exception. Answering no
fetches and hard-resets to origin/master -- the previous behaviour,
still available for deliberately taking upstream changes.

`--reuse-upstream` skips the prompt for unattended installs.

**On uninstall**, the checkout is now KEPT. It lives inside
`/opt/aaa-platform`, which uninstall removes wholesale, so it was being
destroyed along with the application -- meaning every reinstall paid
for a fresh clone. It is someone else's source, not this product's
data. `--purge-upstream` removes it for anyone who wants the disk space
back, and the removal plan printed before confirmation says which it
will do.

**A real bug, caught by testing the preservation rather than assuming
it.** The first implementation checked whether each entry was INSIDE a
preserved path, but `upstream/` is an ANCESTOR of the checkout, not a
descendant -- so it matched nothing and the whole tree was deleted,
checkout included. Precisely the outcome the change exists to prevent.

Fixed by handling all three relationships explicitly: an entry that IS
a preserved path or sits inside one is kept; an entry that CONTAINS one
is recursed into rather than removed; anything else goes. Verified that
the checkout's `.git` and contents survive while `app/`, `venv/`,
`generated/` and loose files are removed, and that a directory with
nothing to preserve is still deleted entirely rather than left as an
empty shell.

Both new flags are threaded through `setup.py` and reach
`bootstrap.sh`, which already forwards its arguments. Every flag the
parser defines is confirmed to be read.

---

### Changed — editions are device-count only; added machine-bound licences

**Tiers now differ by device count and nothing else**: 5 / 15 / 50 /
100 / 300 / unlimited. Every edition includes every feature.

**HA, multi-site, SSO and metered API were removed from the feature
list entirely**, not just ungated. The product does not have them, and
a tier advertising a capability that does not exist is selling
something undeliverable. The keys get added when the capability ships.

**Administrator count is now uncapped in every edition.** An admin
account is not a cost driver, and the previous one-admin Community cap
would have silently disabled NCM Change Control, which refuses
self-approval and therefore needs two people. A limit that breaks a
workflow without saying so is worse than no limit.

### Added — node-locked licences (machine binding)

Built from the idea as described, with the parts that needed
completing.

**Flow**: the customer runs `collect_fingerprint.py` on their server,
sends the resulting `.request` file, and the vendor issues a licence
bound to that machine with `issue_license.py --request`.

**Four components, hashed separately**: `machine_id`, DMI
`product_uuid`, CPU model plus core count, and root filesystem UUID.

**Only hashes leave the machine.** The request file carries SHA-256
digests, never raw values, so a customer emailing it discloses no
UUIDs, addresses or disk layout. `--show` prints exactly what it
contains, so they can verify rather than trust. Confirmed by test that
no raw identifier appears in the file.

**On the "encrypted file" in the original idea -- I did not encrypt
it, and the reason matters.** Encryption would need a key shipped with
the software, which anyone holding a copy could extract; it would look
like protection while providing none. What the file actually needs is
integrity in transit, so it is encoded and checksummed, and a tampered
file is rejected. The system's security rests on the LICENCE being
signed by a key that never leaves the vendor -- someone intercepting a
request learns four hashes and gains nothing.

**Tolerance: 2 of the available components must match.** Binding to
one identifier is brittle -- replace a NIC and a paying customer is
locked out of their own AAA server. Requiring all four is worse. Two
survives ordinary maintenance while still refusing a genuinely
different machine. Verified: a single component change is still
accepted; a foreign machine is not.

**A mismatch falls back to Community with an explanation, never
disabling the platform.** A customer whose motherboard died should not
also lose access to their AAA server.

**The binding is inside the signed payload**, so it cannot be stripped
to make a licence portable -- tested as an explicit attack case.

The issuer refuses to bind a licence when a request carries fewer than
two components, and the collector warns about it BEFORE the customer
sends the file, rather than after the licence fails to work.

The machine id is shown on the License page whether or not a licence is
installed -- it is the value a customer quotes when requesting one, so
it has to be findable before they have one.

---

### Changed — Community now includes Configuration Management

Product decision: the commercial lever is SCALE, not withheld
capability. Community now includes every functional module -- TACACS+,
RADIUS, accounting, Network Operations, Security Center and NCM. Paid
editions differ by device and administrator count, plus API, SSO, HA
and multi-site.

The reasoning is sound: an engineer evaluating the platform sees what
it actually does, and the reason to buy is that their network outgrew
five devices rather than that a feature was held back.

**The single-administrator consequence is surfaced, not buried.** NCM
Change Control refuses self-approval, so its propose -> approve ->
deploy workflow needs two administrators and Community has one.
Everything else in NCM -- backup, archive, diff, drift detection,
baselines -- works fully. A banner on the License page says exactly
that, because the alternative is an operator discovering it at the
moment they try to approve their own change and are refused.

That behaviour falls out of the admin limit rather than a separate
feature check, which is why no code was needed to enforce it.

**The test suite caught the change**, as it should have: the assertion
that NCM was excluded from Community failed. Updated to assert the new
model rather than deleted, so the edition split stays covered.

The licence gating mechanism is unchanged -- it is generic, and
Community simply now holds the feature.

### Reviewed — UI/UX final consistency pass (spec section 30)

Audited rather than assumed, against the measurable criteria:

* Every page has a title and description, except two detail pages
  where the object's own name is the heading -- correct there.
* Every table page has an empty state.
* No icon-only button is missing an `aria-label`.
* Modal sizes remain content-driven: 13 lg / 10 md / 2 xl, not one
  width for everything.
* Escape-to-close and focus containment apply globally to every
  dialog, including ones added later.

No further changes were needed, which is the point of having run the
audit rather than declaring it done.

---

### Added — licence import API, License & Subscription page, security review

Completes the licensing work. The enforcement half shipped first
deliberately; this is the half an operator sees.

**`/platform/license`** shows edition, usage against limits, licence
identity, and EVERY feature with whether it is included -- listing the
excluded ones too, so the difference between editions is visible rather
than implied.

Banners appear only when there is something to act on: an unusable
licence and why, a development-key licence, the device limit reached,
and the grandfathering explanation. Import verifies the signature
BEFORE storing, so pasting the wrong file cannot replace a working
licence with a downgrade.

Removing a licence records the current device count as grandfathered
first, so an installation that drops to Community keeps its devices
working rather than becoming unusable at the moment of removal.

**`tests/test_licensing.py`** -- 12 groups including the attack cases
from the review. Every forgery attempt is asserted REFUSED: payload
edits to device limit, edition, admin limit, expiry, product and
features; a licence signed with an attacker's own key; edited cached
database columns; and malformed input. All pass.

**`docs/LICENSING.md`** carries the security review, including what is
NOT preventable. Anyone with root on self-hosted software can patch the
verification function out, and no obfuscation changes that -- saying
otherwise would be false. What signing does prevent is producing a
licence a clean installation accepts, which is what matters for
distribution and audit. Restoring an old database backup can also reset
the clock high-water mark; detecting that reliably needs a call home,
which would break the air-gapped support this product needs. Both are
recorded as accepted limits rather than left for a customer to discover.

Verified: 51 templates parse; the page renders with zero ID mismatches;
scripts pass syntax checks; all three test suites pass; the issuer tool
is confirmed absent from the distribution archive.

---

### Added — licensing foundation: editions, signed licences, entitlements, enforcement

Analysis first, as asked. The finding that shaped the design: there are
**three independent device-creation paths**, not one --
`routes_devices.py`, `routes_network_scan.py` and
`routes_monitoring.py`. A check in the GUI path alone would have left
two working bypasses.

**Cryptography.** Ed25519 via `cryptography`, already a dependency --
no new package, nothing invented. A licence is
`base64url(payload).base64url(signature)`, signed over the RAW encoded
payload rather than a re-serialisation, so byte-identical verification
is the only thing that passes. The private key lives solely in
`tools/issue_license.py`, which is excluded from the distribution
archive.

A reproducible DEVELOPMENT keypair is published deliberately so the
whole path can be exercised end to end; licences signed with it are
flagged as untrusted, and setting `PRODUCTION_PUBLIC_KEY` stops them
verifying at all.

Verified by execution: a real licence issues and verifies, and every
tamper is rejected -- device limit raised, edition upgraded, expiry
extended, feature added, product changed, plus garbage and empty input.

**Editions live in one file.** `services/editions.py` holds the whole
business model. No module asks "are we Community?"; it asks the
entitlement service for a limit or a feature by key, so changing the
model later is a data change rather than a hunt through the codebase.

Per the product decision: Community is 5 devices, 1 administrator, and
includes TACACS+, RADIUS, accounting, Network Operations and Security
Center. **NCM is paid-only.**

**Limits come from the SIGNED payload, not the edition table.** A
customer who bought "25 devices" keeps 25 even if a later release
redefines Professional.

**Enforcement** at all three creation paths, plus a `before_insert`
backstop on `NetworkDevice` so a fourth path added later fails safe
rather than silently exceeding the licence. The backstop fails OPEN on
any error -- it guards against forgotten code, and a backstop that took
an installation offline over a transient database condition would be
worse than the problem it solves.

**Device counting**: existing rows, including DISABLED ones -- otherwise
"disable to free a slot" is a one-click bypass. Not a high-water mark,
so replacing a failed switch costs nothing. The trade-off is stated
rather than hidden: someone can cycle WHICH devices they manage, never
how many at once, which is what the licence sells.

**Grandfathering**: an installation upgrading with more devices than
Community allows keeps all of them. Nothing is deleted; only growth is
blocked. Deleting a customer's devices to enforce a new licence would
be indefensible.

**An expired or invalid licence falls back to Community and says why**
-- it never disables the platform. Taking a running installation
offline because a renewal is late would be a hostile way to treat a
paying customer.

**Clock rollback** is blunted by a monotonic high-water mark: expiry is
judged against the latest time the installation has ever seen, so
winding the clock back does not revive an expired licence.

**Module gating is per request, not at mount time** -- a licence
imported while the service runs takes effect immediately, and a
customer does not restart to use what they just bought. Unlicensed
modules are also hidden from navigation, because a link that leads
straight to a 402 reads as a broken page rather than a product
boundary. 402 is used rather than 403: this is "your plan does not
include this", not "you lack permission".

Tested across: no licence, at/over limits, admin limit, a Professional
licence unlocking NCM, an expired licence falling back, grandfathering
at 12 devices against a limit of 5, and Enterprise unlimited.

**Still to build**: the licence import API and the License & Subscription
page, plus the written security review. The enforcement half is done and
tested first on purpose -- a page that displays entitlements nothing
enforces would be decoration.

---

### Added — RADIUS Overview; global dialog accessibility and sortable tables

**RADIUS Overview** (`/radius/overview`) is a readiness page rather
than a statistics one, because statistics are still blocked on the
accounting log format. It lists the things that must ALL be true before
a device can authenticate -- RADIUS enabled, ports listening and not
loopback-only, at least one client with a secret, at least one policy
-- each with a link to the page that fixes it.

Two states are kept distinct: a check that FAILED and a check that
could not be RUN. A standard admin cannot read the server settings, and
reporting that as "RADIUS is off" would be a lie told by a permission
boundary. Those rows show "?" and say so.

"Configuration applied" is listed as a reminder, not asserted: this
page cannot tell whether the running daemon matches the database, and
claiming otherwise would be the most misleading thing on the page.

**Global dialog behaviour, added once in `app.js`.** An audit found
**21 dialogs with no Escape handling at all**. Rather than patch 21
files -- which leaves the 22nd to be written with the same gap --
Escape-to-close and Tab focus containment are now delegated from the
document and apply to every `.modal-backdrop` and `.drawer-backdrop`,
including ones created later.

Escape closes only the TOPMOST dialog, since closing a stack at once
would discard work the person cannot see, and it prefers the page's own
close control so existing cleanup runs rather than being bypassed.

**Sortable tables** via `AAAPlatform.makeTableSortable`, opt-in per
table. Deliberately not automatic: policy lists are ordered by
evaluation priority and diff rows by file position, where re-sorting
would destroy the meaning rather than reveal it. Enabled where order
carries no meaning.

**A bug caught by testing the comparator**: `parseFloat` reads
`192.168.44.10` as `192.168`, so every IP address in a column compared
EQUAL and the column silently did not sort. A device table is mostly
addresses, so that was the case that mattered most. IPv4 and CIDR are
now compared by octet value, verified alongside numeric, name-with-digits
(`R2` before `R10`), text, and empty cells.

---

### Added — RADIUS Policies page and API (RADIUS authorization now configurable)

The model and config generation existed and were tested; there was no
way to create a policy. This completes that path, so an authenticated
RADIUS user can actually receive attributes instead of falling back to
the device's own default privilege.

**Conditions** reuse existing objects -- user group (which is how an AD
group is represented here), device, device group, and Service-Type --
rather than a parallel identity model. Any unset condition matches
anything, and a policy with NO conditions is labelled "any RADIUS
request" in amber on the list, because matching everything is easy to
create by accident and important to see at a glance.

**Attribute names are validated against the dictionary tac_plus-ng
itself loads.** Without that check a typo surfaces only when the
configuration is applied -- and a bad directive can stop the daemon
parsing the whole file, taking TACACS+ down with it. Validation is
SKIPPED when the dictionary cannot be read, because refusing every
attribute would be worse than accepting an unverified one.

**Service-Type values come from the dictionary's own enumeration**, so
the dropdown can only offer values the daemon recognises.

**A live preview of the generated profile** is shown in the form. The
compiler remains the single source of truth for what is written; the
preview mirrors it so an operator can check the actual directives
before applying rather than trusting the form.

**Two warnings the obvious implementation would omit:**
* Saving a permit policy with no attributes asks for confirmation --
  it is valid, but the device will use its own default privilege,
  which is rarely what the operator intended.
* The empty state says exactly that consequence, rather than just
  "no policies yet".

**A bug caught before shipping**: the endpoints were written against
`policies:manage`, which does not exist -- the real key is
`policies:write`. Every request would have been denied for every role,
including superadmin. Found by checking the permission catalog rather
than assuming the name.

Verified: 49 templates parse; the page renders with zero ID mismatches;
scripts pass syntax checks; every endpoint the page calls confirmed to
have a matching router; both regression suites still pass.

---

### Fixed — Time column was always empty; added a log-parsing regression suite

Reported from a screenshot: every row in Access Events showed an em
dash for Time.

**Root cause.** `_TIMESTAMP_PATTERN` ends in `\s+` so it can find where
the prefix stops inside a full line -- but `group(1)` EXCLUDES that
whitespace. Re-matching the captured prefix against the same pattern
therefore always failed, and `parsed_at` came back None.

The accounting parser happened to pass the whole LINE rather than the
captured prefix, so it was unaffected. Both parsers written later
passed the prefix, and both were silently broken. Fixed at the root
with a pattern that does not require the trailing whitespace, so all
three formats benefit.

**Why it survived review**: an empty timestamp renders as an em dash,
which reads as "no data" rather than as a failure. Earlier testing of
this parser printed the outcome, user and device -- and never asserted
`parsed_at`. The test checked the fields I was thinking about, not the
ones the feature depended on.

**New `tests/test_log_parsing.py`** to close that gap: every one of the
three formats is asserted to produce a timestamp, using lines captured
from a real deployment. Each case corresponds to a bug that actually
shipped -- the access log's field count, the authorization log's
variable field count, cleartext credential masking, and now the
timestamp.

**Verified the suite catches the bug** by reverting the fix: 8 failures,
naming the exact records whose Time column was empty. A regression test
that cannot fail proves nothing.

---

### Added — real authentication events, from the access log

A sample of `tac_plus-ng-access.log` from a live deployment settled the
format that had been unconfirmed since this project began:

    2026-09-13 05:12:20 +0000 192.168.44.10\tjafar\ttty0\tasync\tshell login failed

Four tab-delimited identity fields, then a free-text message. This is
the AUTHENTICATION outcome -- whether the credential check itself
passed -- which neither of the other logs can answer: accounting
carries no result at all, and authorization describes what an ALREADY
authenticated session was allowed to do.

Access Events now defaults to **Authentication**, reading that log, so
the page answers the question originally asked -- per user, device,
device group, success or failure -- from the log that actually records
it rather than by inference.

**Each event type names its own source on screen.** "Permitted" and
"logged in" are different facts that can disagree: a user can fail
authentication and never reach authorization at all. Labelling the
source stops one being read as the other.

**Outcome derivation stays conservative**: a message must clearly say
it succeeded to count as success. `failed`, `denied`, `rejected` and
`error` are failures; anything unrecognised is UNKNOWN, never success.
An authentication record the platform cannot read must not render as
"logged in successfully". The raw message is shown in its own column
as the evidence the verdict came from.

**A bug caught by testing against the real lines:** the parser
required six fields where the real format has five (four identity plus
message), so every genuine line returned `parsed=False`. Had this been
written from the format description alone and shipped, the
Authentication view would have been empty -- exactly the failure mode
found in the authorization parser one change earlier, for the same
reason: a field count assumed rather than counted.

Verified against all four supplied lines: two successes, two failures,
with `jafar` and `sakjdkasd` correctly identified as the failed
attempts.

---

### Fixed — SECURITY: cleartext credentials exposed; login records never parsed

Both found by testing against a REAL authorization log rather than an
assumed format. Both were introduced by the previous change.

**1. Cleartext credentials were about to be shown in the GUI.** The
real log contains:

    ... permit  shell   username 11 password 22 <cr>

while the accounting log masks the same command as `password *****`.
The authorization log records a command exactly as typed, so switching
the accounting views to that log silently started sourcing cleartext
secrets -- and the new Access Events page would have rendered them in
a table and written them to a CSV export.

`mask_credentials()` now redacts the ARGUMENT of `password`, `secret`,
`key`, `community` and `md5`, applied at the API boundary rather than
in a template, so a CSV export and any future API consumer get the same
protection as the page. Keyword arguments that are not secrets
(`service password-encryption`, `aaa authentication ...`) are left
intact -- masking them would hide the command's meaning while
protecting nothing.

Verified that every command field leaving `routes_accounting` is
masked, by matching on the emitted keys rather than by inspection: the
accounting list and recent-activity endpoints were ALSO exposed, since
they now read the same log, and both were missed on the first pass.

The raw file still contains the secret. That is a file-permissions
matter; the platform simply should not be the thing that copies it onto
someone's screen.

**2. Session (login) records never parsed.** The authorization log has
a VARIABLE field count -- seven for a session authorization, eight when
a command is present:

    ts  nas  user  port  rem_addr  priv  result  service  [cmd]

The accounting parser requires exactly eight, so every login record
returned `parsed=False`. The Access Events page defaults to "session
logins", so it would have shown an empty table -- not an error, just
nothing, which is the worst kind of wrong.

`_auth_parse_line` is now a real parser for that format rather than a
delimiter swap. It also tolerates runs of spaces where a tab was
expected, and rejoins a command containing the separator instead of
truncating it.

Also corrected: positions four and five are the remote-address type
and privilege level, NOT `nac` and `accttype` as the accounting layout
implies. Documented, since the field names are now knowingly
approximate for this log.

Verified against all four real line shapes, plus confirmation that
`::`-delimited accounting parsing is unaffected -- including that the
accounting log's `result` field is genuinely EMPTY, which is the
evidence behind reading the authorization log in the first place.

---

### Added — Access Events page: filter access decisions by user, device, group and outcome

Requested: filter authentication per user / device / group / success or
failure.

**Source chosen on evidence, and named accurately.** The obvious source
would be `tac_plus-ng-access.log`, but its line format has never been
confirmed for this project -- it is served raw elsewhere for exactly
that reason, and parsing it would mean guessing at field positions in a
security record. The AUTHORIZATION log is parsed, is now known to be
tab-delimited with the same eight fields, and carries the permit/deny
decision -- including the initial shell authorization, which IS the
"may this user get a session on this device" decision.

So the page is called **Access Events** rather than Authentication.
Calling it Authentication would promise the credential check itself,
which this data does not contain.

**`read_auth_records(include_logins=True)`** -- the existing reader
drops records with an empty command, which is right for a command list
but discards precisely the session decisions this view needs. The
filter is now optional rather than duplicated into a second reader.

**Filters**: event type (session logins / command decisions / all),
outcome, user, device, device group, time range and free text. Export
writes the FILTERED set.

**Two correctness decisions:**
* `login_outcome` treats anything not recognisably a permit as a
  FAILURE, and a record with no result at all as UNKNOWN -- a third
  state, not folded into either. An unreadable access decision must
  never be presented as access granted, and "no data" is not "denied".
  The raw `result` field is shown in its own column so an operator can
  see what the verdict was derived from.
* The success/failure/unknown counts are computed BEFORE the outcome
  filter is applied, so selecting "failure" still shows how many
  successes exist alongside it. Counting afterwards would make the
  breakdown agree with itself and tell the reader nothing.

**A bug caught in my own code before shipping**: the counts were
originally incremented after the outcome filter, while the comment
above them claimed the opposite. Comment and code now agree, and an
assertion checks the ordering rather than trusting the comment.

Verified across seven filter cases including partial user and device
matches, device-group membership, command-versus-login separation, and
that an unreadable result is not counted as success.

---

### Changed — accounting views now read the AUTHORIZATION log (user-contributed), parsers merged

Adopted a change made directly against a real deployment: the
accounting list and AAA-health views now read
`tac_plus-ng-authorization.log` instead of the accounting log.

The reasoning is sound and worth recording. The accounting log
contains commands that were ALLOWED -- it cannot record a denial, and
does not even state that a logged command was permitted. A dashboard
built on it therefore describes a world in which nothing is ever
refused. The authorization log carries the actual decision, permit and
deny alike, in the same eight fields but TAB-delimited.

**What I changed on top: the two parsers are now one.**
`_auth_parse_line` was a 99.5% copy of `_parse_line`, differing only in
the delimiter. A fix to one would silently have missed the other,
which is the classic way parsing bugs return. `_parse_line` now takes
the separator as a parameter and `_auth_parse_line` is a three-line
wrapper.

**Behavioural equivalence was proven, not assumed**: both versions run
side by side over accounting lines, authorization lines, a line with
no timestamp prefix, a partial-field line and unparseable input --
identical results on all six.

**Two bugs in my own refactor, caught by that test.** The default
argument referenced `FIELD_SEPARATOR`, which does not exist (the
constant is `ACCOUNTING_FIELD_SEPARATOR`), so the module would have
failed to import -- the service would not have started. And a
subsequent string replacement produced `ACCOUNTING_separator.join`,
mangling a real identifier. `py_compile` passed both times; only
executing the parser found them. That is the third time this session
compilation has proven weaker evidence than execution.

Documented in `docs/ARCHITECTURE.md`, including the honest limitation:
the authorization log's tab layout is OBSERVED from a real deployment,
not declared by a directive this project emits. If upstream changes
that default, parsing needs revisiting -- and no `authorization format`
directive was confirmed in the samples, so inventing one would risk a
configuration the daemon rejects.

---

### Added — RADIUS listener status check (bind address, not just "is it configured")

Prompted by a real diagnostic session: `tcpdump` showed no packets
arriving during a MikroTik login, which no RADIUS configuration change
could ever fix. That is a bind-address or network-path question, and
the platform could not answer it.

`GET /api/radius/listeners` reports, for each configured port, whether
anything is listening and on WHICH address, distinguishing four states:
nothing listening, loopback-only, one specific address, or all
interfaces.

**The loopback-only case is the one worth having.** A daemon bound
only to loopback answers a local probe perfectly while being
unreachable from every other host -- so a NAS gets nothing and its
packets never even reach a capture on this server. "The built-in test
passes but my switch gets no reply" is exactly that shape, and it is
indistinguishable from a RADIUS problem without this.

Read from `/proc/net/udp` rather than shelling out to `ss`: this runs
as an unprivileged service account, `ss` may not be on PATH, and
adding a sudo rule for a read-only diagnostic would widen the
service's privileges for no reason. `/proc/net` is world readable.

Parsing verified against the kernel's actual little-endian hex format
(`0.0.0.0`, `127.0.0.1` and a specific address all decoded correctly,
wrong-port rows ignored), and the four status classifications tested
individually.

Surfaced on the RADIUS Server page ABOVE the probe, because a
loopback-only or absent listener makes every other RADIUS question
moot.
