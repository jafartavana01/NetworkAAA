...
def phase_build_tac_plus_ng(report: system_checks.SystemReport, *, reuse_upstream: bool = False) -> dict:
    utils.header("tac_plus-ng -- Download, Configure, Build, Install")

    commit = upstream_build.clone_or_update_upstream(assume_yes=reuse_upstream)
...
def main(force_config: bool = False, reuse_upstream: bool = False) -> None:
    try:
        ...
        phase_dependencies()
        build_info = phase_build_tac_plus_ng(report, reuse_upstream=reuse_upstream)
        phase_database()
...
def run_uninstall(*, force: bool, keep_logs: bool, purge_upstream: bool = False) -> None:
    try:
        uninstall.run_uninstall(force=force, keep_logs=keep_logs, purge_upstream=purge_upstream)
    except utils.InstallError as exc:
        utils.error(str(exc))
        utils.info(f"See {utils.LOG_PATH} for the full command log.")
        sys.exit(1)
...
def parse_args(argv: list[str]):
    ...
    parser.add_argument(
        "--purge-upstream", action="store_true",
        help=("Also delete the upstream source checkout during uninstall. It is kept by "
              "default so the next install does not download it again."),
    )
    parser.add_argument(
        "--reuse-upstream", action="store_true",
        help=("Reuse an existing upstream checkout without prompting. Useful for "
              "unattended installs on hosts with no internet access."),
    )
    parser.add_argument(
...
if __name__ == "__main__":
    ...
    if args.uninstall:
        run_uninstall(force=args.force, keep_logs=args.keep_logs,
                      purge_upstream=args.purge_upstream)
    else:
        main(force_config=args.force_config, reuse_upstream=args.reuse_upstream)
